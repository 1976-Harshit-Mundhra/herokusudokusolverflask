...
def call_solver(imgpath):
    ...
    # print(os.getcwd())
    #puzzle = cv2.imread(os.path.join(os.getcwd(), imgpath.split('/')[-1]))

    # Resizing puzzle to be solved
    puzzle = cv2.resize(imgpath, (450,450))
    # try:
...
class sudokusolver(Resource):
    def get(self, url):
        ...
        img_encoded = np.asarray(bytearray(req.read()), dtype=np.uint8)    
        img = cv2.imdecode(img_encoded, cv2.IMREAD_COLOR)
        unsol, sol = call_solver(img)
        return ({'solved':sol,'unsolved':unsol})

api.add_resource(status, '/')
api.add_resource(sudokusolver, '/sudokusolver/<path:url>/')